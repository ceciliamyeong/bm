...
# Funding disabled by default (can be enabled with BM20_ENABLE_FUNDING=1)
ENABLE_FUNDING = os.getenv("BM20_ENABLE_FUNDING", "0") == "1"
if not ENABLE_FUNDING:
    print("[MODE] Funding rate collection disabled.")
...
def get_kimchi(df):
    ...
    # ---- USDKRW (more robust) ----
    def _valid_usdkrw(x: float) -> bool:
        try:
            x = float(x)
            return np.isfinite(x) and (900 <= x <= 2000)
        except Exception:
            return False

    last_cache = read_json(KP_CACHE) or {}
    ...
    if not _valid_usdkrw(usdkrw):
        try:
            fxj = _get("https://open.er-api.com/v6/latest/USD")
            usdkrw = float(fxj["rates"]["KRW"])
            fx = "open.er-api.com"
            if not _valid_usdkrw(usdkrw):
                raise ValueError
    ...
    if not _valid_usdkrw(usdkrw):
        try:
            fxj = _get("https://api.frankfurter.app/latest", {"from": "USD", "to": "KRW"})
            usdkrw = float(fxj["rates"]["KRW"])
            fx = "frankfurter.app"
            if not _valid_usdkrw(usdkrw):
                raise ValueError
    ...
    if not _valid_usdkrw(usdkrw):
        try:
            cached = float(last_cache.get("usdkrw"))
            if _valid_usdkrw(cached):
                usdkrw = cached
                fx = "cache:usdkrw"
        except Exception:
            pass

    # 6) final hard fallback
    if not _valid_usdkrw(usdkrw):
        usdkrw = 1450.0
        fx = "fixed1450"
...
def _cache_is_valid(bj: dict) -> bool:
    try:
        if not isinstance(bj, dict):
            return False
        bd = str(bj.get("base_date", "")).strip()
        if not bd.startswith("2018-"):
            return False
        bv = float(bj.get("base_value", 0))
...
base_date = BASE_DATE_TARGET
base_value = None

if BASE_FILE.exists():
    bj = read_json(BASE_FILE) or {}
    if _cache_is_valid(bj):
        ...
        write_json(BASE_FILE, {"base_date": base_date, "base_value": base_value, "note": "recomputed_invalid_cache"})
else:
    base_date, base_value = _calc_base_value(df["id"].tolist(), weights_map)
...
# Funding rates disabled by default. Set BM20_ENABLE_FUNDING=1 to enable collection.
BIN_TEXT = None
BYB_TEXT = None
...
def build_news_editorial():
    # ✅ SSOT: always use root bm20_latest.json if available (keeps news consistent with dashboard)
    try:
        j = read_json("bm20_latest.json") or {}
        if isinstance(j, dict) and ("bm20Level" in j) and ("bm20ChangePct" in j):
            bm20_now = float(j["bm20Level"])
            bm20_chg = float(j["bm20ChangePct"])
            # Prefer kimchi from SSOT as well (if present)
            if "kimchi_premium_pct" in j and j["kimchi_premium_pct"] is not None:
                kimchi_pct = float(j["kimchi_premium_pct"])
            elif "kimchi" in j and j["kimchi"] is not None:
                # kimchi may be ratio (e.g., 0.0291) or pct (2.91); normalize to pct
                kv = float(j["kimchi"])
                kimchi_pct = kv * 100.0 if kv < 1 else kv
            # Prefer asOf date for headline consistency
            if j.get("asOf"):
                ymd = str(j["asOf"])
    except Exception:
        pass
...
story += [card([Paragraph("BM20 데일리 뉴스", section_h), Spacer(1,2), Paragraph(news.replace("\n","<br/>"), body_style)]),
          Spacer(1, 0.45*cm)]
story += [Paragraph("© Blockmedia · Data: Yahoo Finance, Upbit",
                    small_style)]
doc.build(story)
...
<div class="wrap">
  ...
  <div class="footer">© Blockmedia · Data: Yahoo Finance, Upbit</div>
</div></body></html>
""")
