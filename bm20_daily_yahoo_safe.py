...
def _yf_price_on_date(yf_ticker: str, date_ymd: str):
    """
    Yahoo chart v8에서 period1/period2를 쓰지 않고 range=max로 전체 히스토리를 받는다.
    - 해당 날짜가 없으면 '최초 가용 종가'로 폴백
    - 오류(result:null/400 등)는 None 반환
    """
    try:
        j = _get(f"https://query2.finance.yahoo.com/v8/finance/chart/{yf_ticker}", {"range": "max", "interval": "1d"})
        ch = j.get("chart", {})
        if ch.get("error"):
            return None
        ...
        cl = quotes[0].get("close") if quotes else []
        if not ts or not cl:
            return None
        target = datetime.fromisoformat(date_ymd).date()
        for t, v in zip(ts, cl):
