...
syms = [s for s in m["symbol"].unique()
        if inc.get(s,True) and s not in STABLE|DERIV|EXC]
...
# 기본 가중치: 시가총(없으니 균등) → KR 보너스 → BTC/ETH cap → 정규화
...
for _ in range(12):
    ...
    if pool>0: w.loc[others]+=w.loc[others]/pool*exc
    w = w/w.sum()
def contrib_sum(df):
    # 가용종목만 재정규화한 일간 기여 합
    c = {}
    for d,row in rets.loc[df.index].iterrows():
        avail=row.dropna()
...
mt0=pd.Timestamp(year=last.year, month=last.month, day=1)
qt0=pd.Timestamp(year=last.year, month=((last.month-1)//3)*3+1, day=1)
yt0=pd.Timestamp(year=last.year, month=1, day=1)
out={
  "asof": str(last.date()),
  "MTD": contrib_sum(prices.loc[prices.index>=mt0]),
  "QTD": contrib_sum(prices.loc[prices.index>=qt0]),
  "YTD": contrib_sum(prices.loc[prices.index>=yt0]),
}
