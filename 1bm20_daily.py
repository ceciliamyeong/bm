...
# - CoinGecko 호출: **Pro(API 키) 우선 → Demo 키 → Public** 순으로 자동 선택
# - market_chart 실패 시 경고만 내고 빈 시리즈 처리(리포트 생성 지속)
# - KRW_BONUS를 환경변수 BM20_KRW_BONUS로 제어(기본 1.0)
...
# --- CoinGecko 공통 (Pro 키 우선 → 데모 → 퍼블릭) ---
CG_API_KEY  = os.getenv("COINGECKO_API_KEY")           # Pro/API key
CG_DEMO_KEY = os.getenv("COINGECKO_DEMO_API_KEY")      # (optional) demo key
...
if CG_API_KEY:
    CG_HEADERS["x-cg-pro-api-key"] = CG_API_KEY  # Pro 헤더

def cg_get(path, params=None, retry=8, timeout=20):
    """CoinGecko 호출: Pro(API 키) 우선, 없으면 퍼블릭(+데모 키 파라미터)"""
    last = None
...
def get_kp(df):
    def _req(url, params=None, retry=5, timeout=12, headers=None):
        last=None
    ...
    try:
        ...
        try:
            p={"ids":"bitcoin","vs_currencies":"krw"}
            if not CG_API_KEY and CG_DEMO_KEY: p["x_cg_demo_api_key"]=CG_DEMO_KEY
            cg=_req(f"{CG_BASE}/simple/price", p, headers=CG_HEADERS)
    ...
    if btc_usd is None:
        try:
            ...
            try:
                p={"ids":"bitcoin","vs_currencies":"usd"}
                if not CG_API_KEY and CG_DEMO_KEY: p["x_cg_demo_api_key"]=CG_DEMO_KEY
                cg=_req(f"{CG_BASE}/simple/price", p, headers=CG_HEADERS)
    ...
    try:
        p={"ids":"tether","vs_currencies":"krw"}
        if not CG_API_KEY and CG_DEMO_KEY: p["x_cg_demo_api_key"]=CG_DEMO_KEY
        t=_req(f"{CG_BASE}/simple/price", p, headers=CG_HEADERS)
...
plt.bar(x, y, width=0.82, edgecolor="#263238", linewidth=0.2, color=colors_v)
...
def safe_get_pct_series(coin_id, days=8):
    try:
        ...
        if not CG_API_KEY and CG_DEMO_KEY:
            p["x_cg_demo_api_key"] = CG_DEMO_KEY
        data = requests.get(f"{CG_BASE}/coins/{coin_id}/market_chart", params=p, timeout=20, headers=CG_HEADERS).json()
