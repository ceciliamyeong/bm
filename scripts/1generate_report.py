...
# BM20 데일리 리포트: out/* 산출물과 루트 파일을 읽어 bm20_latest.html 생성
# - 데이터가 없어도 절대 크래시하지 않음(빈 섹션 처리)
# - perf_up/down, contrib_top, latest(김치/펀딩) 스키마 유연 대응
# - 경로 정책: "out/"와 "루트"만 탐색 (bm/ 프리픽스 사용 안 함)

import os, json, datetime, glob
from pathlib import Path

ROOT = Path(".")
OUT_DIR = ROOT / "out"       # 산출물 최상위
...
# ----------------- 경로 탐색( out + 루트 ) -----------------
def list_out_date_dirs(max_keep=8):
    """out/20*/ 형태의 날짜 디렉토리를 정렬하여 최신 max_keep개만 반환"""
    if not OUT_DIR.exists():
        return []
    dirs = [p for p in OUT_DIR.glob("20*") if p.is_dir()]
    dirs.sort(key=lambda p: p.name)  # YYYY-MM-DD 기준
    return dirs[-max_keep:] if max_keep else dirs

def load_out_or_root_json(filename: str, default=None):
    """
    우선순위:
      1) out/latest/filename
      2) out/YYYY-MM-DD/filename (최신 날짜부터 역순으로)
      3) 루트/filename
      4) site/filename (있을 때)
    """
    cands = []
    cands.append(str(OUT_DIR / "latest" / filename))

    for d in reversed(list_out_date_dirs(max_keep=12)):
        cands.append(str(d / filename))

    cands.append(str(ROOT / filename))
    cands.append(str(ROOT / "site" / filename))
...
def latest_json_candidates():
    """
    latest.json / bm20_latest.json에 대한 후보 목록 생성
    우선순위:
      1) 루트 latest.json, bm20_latest.json
      2) out/latest/latest.json, out/latest/bm20_latest.json
      3) out/YYYY-MM-DD/latest.json (최신 역순)
      4) out/YYYY-MM-DD/bm20_latest.json (최신 역순)
      5) site/latest.json
    """
    cands = [
        "latest.json",
        "bm20_latest.json",
        str(OUT_DIR / "latest" / "latest.json"),
        str(OUT_DIR / "latest" / "bm20_latest.json"),
    ]
    for d in reversed(list_out_date_dirs(max_keep=12)):
        cands.append(str(d / "latest.json"))
    for d in reversed(list_out_date_dirs(max_keep=12)):
        cands.append(str(d / "bm20_latest.json"))

    cands.append(str(ROOT / "site" / "latest.json"))
    return cands

def find_images(asof_date: str | None):
    """
    이미지 후보를 날짜형과 latest alias 순으로 탐색
    우선순위:
      1) out/YYYY-MM-DD/bm20_bar_YYYY-MM-DD.png 등 (asof가 있으면)
      2) out/latest/*_latest.png
      3) 루트/*_latest.png
    """
    names_latest = [
        "bm20_bar_latest.png",
        "bm20_over_btc_latest.png",
        "bm20_over_eth_latest.png",
        "bm20_btc_eth_line_latest.png",
        "kimchi_premium_latest.png",
    ]
    ...
    # 1) 날짜 기반 이미지 (있는 경우)
    if asof_date:
        ymd = str(asof_date)[:10]
        names_by_date = [
            f"bm20_bar_{ymd}.png",
            f"bm20_over_btc_{ymd}.png",
            f"bm20_over_eth_{ymd}.png",
            f"bm20_btc_eth_line_{ymd}.png",
            f"kimchi_premium_{ymd}.png",
        ]
        d = OUT_DIR / ymd
        for n in names_by_date:
            cand_paths.append(d / n)

    # 2) out/latest alias
    for n in names_latest:
        cand_paths.append(OUT_DIR / "latest" / n)

    # 3) 루트 alias
    for n in names_latest:
        cand_paths.append(ROOT / n)

    return [p for p in cand_paths if exists(p)]

# ----------------- 메인 -----------------
def main():
    # 1) 데이터 로드: out/ + 루트
    perf_up_raw   = load_out_or_root_json("perf_up.json", {})
    perf_down_raw = load_out_or_root_json("perf_down.json", {})
    contrib_raw   = load_out_or_root_json("contrib_top.json", {})

    latest_raw = load_first(latest_json_candidates())
    ...
    asof = pick_date(date_up, date_down, contrib.get("asof"), latest_raw.get("asof"))
    ...
    # 3) 이미지 자동 삽입(있을 때만)
    imgs = find_images(asof)
    imgs_html = ""
    if imgs:
        parts = [f'<img src="{p.as_posix()}" alt="{p.name}"/>' for p in imgs]
