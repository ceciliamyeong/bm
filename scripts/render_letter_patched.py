...
- out/history/btc_usd_series.json
- data/bm20_history.json (sentiment)
- out/global/k_xrp_share_24h_latest.json (XRP KR share)
- out/latest/news_one_liner.txt (optional)
- out/latest/news_one_liner_note.txt (optional, manual)
...
# NEW (confirmed by you)
BM20_HISTORY_JSON = ROOT / "data/bm20_history.json"
XRP_KR_SHARE_JSON = ROOT / "out/global/k_xrp_share_24h_latest.json"
...
def compute_moves_top3(df: pd.DataFrame):
    """For MOVE_1~3 block: top 3 gainers by 1D change."""
...
# ------------------ extra signals (sentiment, xrp share) ------------------

def extract_sentiment(obj) -> tuple[str, str]:
    """
    bm20_history.json may evolve. We try common shapes/keys.
    Returns: (label, score_text)
    """
    label = None
    score = None
    ...
    if isinstance(obj, dict):
        # direct keys
        for k in ("sentiment_label", "sentimentLabel", "label", "market_sentiment_label"):
            if k in obj and obj.get(k) is not None:
                label = obj.get(k)
                break
        for k in ("sentiment_score", "sentimentScore", "score", "market_sentiment_score"):
            if k in obj and obj.get(k) is not None:
                score = obj.get(k)
                break
        ...
        if latest:
            if label is None:
                for k in ("sentiment_label", "sentimentLabel", "label"):
                    if k in latest and latest.get(k) is not None:
                        label = latest.get(k); break
            if score is None:
                for k in ("sentiment_score", "sentimentScore", "score"):
                    if k in latest and latest.get(k) is not None:
                        score = latest.get(k); break

        # nested series: last element
            ...
            if label is None:
                for k in ("sentiment_label", "sentimentLabel", "label"):
                    if k in last and last.get(k) is not None:
                        label = last.get(k); break
            if score is None:
                for k in ("sentiment_score", "sentimentScore", "score"):
                    if k in last and last.get(k) is not None:
                        score = last.get(k); break
...
def extract_xrp_kr_share(obj) -> str:
    if not isinstance(obj, dict):
        return "—"

    # common fields
    for k in ("xrp_kr_share", "xrp_kr_share_pct", "share_pct", "share", "value"):
        if k in obj and obj.get(k) is not None:
            try:
                return fmt_share_pct(float(obj.get(k)))
            except Exception:
    ...
    if isinstance(latest, dict):
        for k in ("xrp_kr_share", "xrp_kr_share_pct", "share_pct", "share", "value"):
            if k in latest and latest.get(k) is not None:
                try:
                    return fmt_share_pct(float(latest.get(k)))
                except Exception:
                    return str(latest.get(k))
    return "—"
...
def build_placeholders() -> dict[str, str]:
    ...
    # BTC: use series last two
    series = load_json(BTC_JSON)
    if not isinstance(series, list) or len(series) < 2:
        raise ValueError("btc_usd_series.json must be a list with at least 2 points")
    ...
    if kimchi_p is not None:
        kimchi_pct = float(kimchi_p)  # percent
    ...
    # KRW totals + shares
    ts_label = krw.get("timestamp_label", "")  # already KST label in your generator
    ...
    # Moves 3 (top gainers)
    m1, m2, m3 = compute_moves_top3(df)

    # Sentiment
    sentiment_label, sentiment_score = ("—", "—")
    hist_obj = load_json_optional(BM20_HISTORY_JSON)
    if hist_obj is not None:
        sentiment_label, sentiment_score = extract_sentiment(hist_obj)
    ...
    return {
        ...
        "{{KRW_ASOF_KST}}": ts_label if ts_label else str(asof)[:10],
...
def render():
    ...
    # sanity: find leftovers like {{SOMETHING}}
    leftovers = sorted(set(re.findall(r"\{\{[A-Z0-9_]+\}\}", html)))
    if leftovers:
        print("WARN: Unfilled placeholders:", leftovers)
