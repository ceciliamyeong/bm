...
# - 김치 프리미엄/펀딩비 표기 (Upbit/Binance/환율)
# - 뉴스(조선비즈 톤) 자동 생성
# - 차트 overflow 방지(이미지 안의 한글 제거, 제목은 HTML로)
...
def kimchi_premium_btc():
    try:
        ...
        return prem, {"upbit_krw":up, "usdkrw":fx, "binance":binance}
    except Exception:
        return None, None
...
# BTC/ETH 한 줄용(우리 DF 컬럼명에 맞춤: pct/price)
btc_row = df_all[df_all["symbol"]=="BTC"].head(1)
eth_row = df_all[df_all["symbol"]=="ETH"].head(1)
...
def build_news():
    ...
    if len(btc_row):
        br = btc_row.iloc[0]
        btc_line = f"비트코인(BTC) {pct(br['pct'])}, {num(br['price'])}달러."
    eth_line = ""
    if len(eth_row):
        er = eth_row.iloc[0]
        eth_line = f"이더리움(ETH) {pct(er['pct'])}, {num(er['price'])}달러."
...
h2{{font-size:18px;color:#123b8a;margin:8px 0}} h3{{font-size:20px;margin:6px 0}}
...
print("Saved. Kimchi:", kimchi_pct, "Funding:", fund_btc, fund_eth)
