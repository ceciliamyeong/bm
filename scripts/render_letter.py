...
Recommended repo layout (paths are repo-root relative):
...
# Always resolve paths from repo root even when the script lives in scripts/
...
def load_daily_df() -> pd.DataFrame:
    ...
    if "symbol" not in df.columns or "price_change_pct" not in df.columns:
        raise KeyError("Daily CSV must contain 'symbol' and 'price_change_pct' columns (or compatible aliases).")
...
def load_btc_price_and_1d():
    if not BTC_JSON.exists():
        raise FileNotFoundError(f"Missing {BTC_JSON}. Expected BTC series at out/history/btc_usd_series.json")
    btc_series = json.loads(BTC_JSON.read_text(encoding="utf-8"))
    if not isinstance(btc_series, list) or len(btc_series) < 2:
        raise ValueError("btc_usd_series.json must be a list with at least 2 points to compute 1D change.")

    last = btc_series[-1].get("price")
    prev = btc_series[-2].get("price")
    if last is None or prev is None:
        raise KeyError("btc_usd_series.json points must include 'price'.")

    btc_usd = float(last)
    btc_1d = (btc_usd / float(prev) - 1.0) * 100.0

    btc_price_txt = f"{btc_usd:,.0f}"
    btc_1d_txt = f"{btc_1d:+.2f}%"
    return btc_price_txt, btc_1d_txt
...
def build_placeholders() -> dict[str, str]:
    ...
    btc_price_txt, btc_1d_txt = load_btc_price_and_1d()
    ...
    if combined is not None and float(combined) > 0 and upbit_v is not None:
        upbit_share = float(upbit_v) / float(combined) * 100.0
    if combined is not None and float(combined) > 0 and bith_v is not None:
        bith_share = float(bith_v) / float(combined) * 100.0
    if combined is not None and float(combined) > 0 and coin_v is not None:
...
def render():
    if not TEMPLATE.exists():
        raise FileNotFoundError(f"Missing {TEMPLATE}. Place letter_newsletter_template.html at repo root.")
