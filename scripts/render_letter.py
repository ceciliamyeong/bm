...
def fetch_coingecko_ticker() -> dict[str, str]:
    """
    CoinGecko /simple/price 로 BTC·ETH·XRP 현재가 + 24h 변동률 조회.
    실패 시 "—" 로 채운 dict 반환 (뉴스레터 발송은 계속 진행).
    """
    try:
        resp = requests.get(
        ...
        fallback = {"PRICE": "—", "CHANGE": "—", "COLOR": "ticker-down"}
        return {
            **{f"TICKER_BTC_{k}": v for k, v in fallback.items()},
            **{f"TICKER_ETH_{k}": v for k, v in fallback.items()},
            **{f"TICKER_XRP_{k}": v for k, v in fallback.items()},
            "TICKER_TIME": _kst_now(),
        }

    def _fmt(coin_id: str, symbol: str) -> dict[str, str]:
        d = data.get(coin_id, {})
        price  = d.get("usd")
        ...
        if change is None:
            change_str = "—"
            color_cls  = "ticker-down"
        else:
            arrow      = "▲" if change >= 0 else "▼"
            color_cls  = "ticker-up" if change >= 0 else "ticker-down"
            change_str = f"{arrow}{abs(change):.1f}%"

        return {
            f"TICKER_{symbol}_PRICE":  price_str,
            f"TICKER_{symbol}_CHANGE": change_str,
            f"TICKER_{symbol}_COLOR":  color_cls,
        }

    result = {}
    result.update(_fmt("bitcoin",  "BTC"))
    result.update(_fmt("ethereum", "ETH"))
    result.update(_fmt("ripple",   "XRP"))
...
def build_placeholders() -> dict[str, str]:
    ...
    # ── 티커 바: CoinGecko 실시간 + BM20 변동률 ──
    ticker = fetch_coingecko_ticker()
