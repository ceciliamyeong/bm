...
def build_placeholders() -> dict[str, str]:
    ...
    best3, worst3, breadth, up, down = compute_best_worst_breadth(df)
    ...
    if r1d is not None:
        ...
        if abs(v) <= 1.5:
            v *= 100.0
        direction = "반등" if v > 0 else ("약세" if v < 0 else "보합")
    ...
    kimchi_txt = f"{float(kimchi_p):+.2f}%" if kimchi_p is not None else "—"
