# contrib_report.py
# 기여도(MTD/QTD/YTD) → api/contrib_top.json
# 1D 성과 → api/perf_up.json / api/perf_down.json
...
ap.add_argument("--out-dir", default="api")        # ✅ 루트 api/ 고정
ap.add_argument("--out-json", default=None)        # (호환) 주면 디렉터리만 추출
args = ap.parse_args()

if args.out_json and args.out_dir == "api":
...
if "listed_kr_override" in m.columns:
    ...
    bonus = pd.Series(1.0, index=w.index); bonus.loc[bonus.index.map(lambda s: lk.get(s, False))] = args.listed_bonus
    w = w * bonus
...
for _ in range(12):
    ...
    pool = float(w.loc[others].sum()) if others else 0.0
    if pool > 0:
        w.loc[others] += w.loc[others] / pool * exc
    w = w / w.sum() if w.sum() > 0 else w

def contrib_sum(idx: pd.DatetimeIndex) -> dict:
    if prices.empty or len(idx) == 0: return {}
    sub = prices.loc[idx]
    rets = sub.pct_change(fill_method=None)  # FutureWarning 제거
    c = {}
...
(out_dir / "contrib_top.json").write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
...
try:
    if not prices.empty and len(prices.index) >= 2:
        ...
        def mk(items): return [{"symbol": sym, "ret_24h_pct": round(float(v), 4)} for sym, v in items]
        perf_up   = mk(ret1d.head(10).items())
        perf_down = mk(ret1d.tail(10).items())
except Exception as e:
    print("[WARN] daily perf build skipped:", e)

(out_dir / "perf_up.json").write_text(json.dumps({"date": out.get("asof"), "top": perf_up}, ensure_ascii=False, indent=2), encoding="utf-8")
(out_dir / "perf_down.json").write_text(json.dumps({"date": out.get("asof"), "bottom": perf_down}, ensure_ascii=False, indent=2), encoding="utf-8")
